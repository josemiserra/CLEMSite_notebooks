    ...
    for el in m_indexes:
             ...
             for col in data_calc.select_dtypes(include=numerics):
                 if col!=hue and col!='index':
                         ...
                         if remove_outliers:
                             data_calc = data_calc[np.abs(data_calc[col_zscore])< np.abs(data_calc[col_zscore].mean()+3*data_calc[col_zscore].std())]
             final_df = pd.concat([final_df,data_calc])
...
def plotNselect_tSNE(df_feat,features, size=10, perplexity = 80, ncomp = 2):
    ...
    Y = TSNE(perplexity=perplexity, n_components=ncomp).fit_transform(df_i[features])

    options = dict(legend_position='left', width=1000, height=600,
                       scaling_method='width', scaling_factor=2, size_index=2, show_grid=True,
                       tools=['hover', 'box_select', 'lasso_select'], line_color='k', cmap='Accent', size=size,
