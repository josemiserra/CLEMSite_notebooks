function runLD_LM(input_image,output_folder,parameters_filename,image_metadata,scripts_folder,true_letter)
    ...
    %               recommended, for LM 0.075 is nice parameter.
    %               
    ...
    try
        ...
        softParams.sigma = str2double(data.preferences.line_detection{1}.LM{1}.gaussian);
        softParams.clahe = str2double(data.preferences.line_detection{1}.LM{1}.clahe);
        softParams.canthresh = str2double(data.preferences.line_detection{1}.LM{1}.canny_thresh);
        ...
        softParams.strokeWidth = str2double(data.preferences.line_detection{1}.LM{1}.stroke);
        ...
        if(~isempty(image_metadata))        
           ...
           center_point(1,1) =  dataIm.PositionY; % Remember, microscope is inverted
           center_point(1,2) =  dataIm.PositionX; 
           orientation = dataIm.orientation; % How much I need to rotate to orient my pattern in a way that the first point is the top bottom right
           
           
           tpoint(1,1) =  dataIm.TargetPositionY; % Remember, microscope is inverted
           tpoint(1,2) =  dataIm.TargetPositionX; 
           
           % templates for the recognition
           cbdir = strcat(scripts_folder,'/codebook/');
           
           [flist,hint] = identify_pattern_LM(images{5},info.cutpoints,distsq,lfolder,orientation,info.centrePointIndex,dataIm.letter_center,cbdir,true_letter);
           ...
           inpoints = imageToStageCoordinates_LM(inpoints,center_point,img,dataIm.PixelSize*1e-3);           
    else
    ...
    filename = strcat(folder,filename);
    together = [info.centrePointIndex; info.angneg; info.angpos; length(inpoints)];
    ...
    filename = strcat(tag,'_fcoordinates_target.xml');
    filename = strcat('/',filename);
    filename = strcat(output_folder,filename);
    tname = strcat('cell_',tag);
    tmappoint = getTransformedPoint(inpoints,mappoints,tpoint);
    tname = strcat(tname,num2str(tmappoint(1)));
    tname = {strcat(tname,num2str(tmappoint(2)))};
    saveCoordinatesXML(tpoint,tmappoint,tname,filename);
catch ME
      getReport(ME)
end