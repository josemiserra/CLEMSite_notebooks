function [finalSet]= fillPatternNames_LM(cutpoints,pattern,icut,ang)
    ...
    %  we associate a list of letter names
       ...
       [cpoints,imset] = sortSquarePoints_LM(cpoints,ang);
       ...
       while(~isempty(to_check))
         ...
         [cpoints,imset] = sortSquarePoints_LM(gpoint,ang);
         ...
         if(~isempty(set1))
            ...
            if(poslplus-1 == 0)
                ...
                if(~vl(set1))% if the group is not complete, then complete it
                    [cpointsN,imset] = sortSquarePoints_LM(cpointsN,ang);
