...
%--  Divide between positive and negative angles 
      ...
      if(error>0)
          if(error==1)
            ...
            [goodpeaks,error] = predictGrid(anglepos,R,gridsize);
            return;
          end
          if(error ==2)
             angleneg = ipeaks(anglenegind,:);
             angleneg = sortrows(angleneg,1);
            [goodpeaks,error] = predictGrid(angleneg,R,gridsize);
            return;
          end
          return;
      end
     ...
     angleneg = angleneg(good_ind_neg,:);
      ...
      [positivePairs,~] = findGridSequence(anglepos,gridsize);
      [negativePairs,~] = findGridSequence(angleneg,gridsize);
...
function [error] = enoughpeaks(pospeaks,negpeaks)
     ...
     if(length(pospeaks)<4 && length(negpeaks)>=4)
             error = 2;
             return;
     end
