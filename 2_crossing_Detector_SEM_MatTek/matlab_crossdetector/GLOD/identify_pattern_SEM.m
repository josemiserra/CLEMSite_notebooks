function [flist,epattern,prob_pattern]= identify_pattern_SEM(iimg,all_cutpoints,distsq,folder,orientation,indpattern,epattern,codebookdir,true_pattern)
%%
...
icpattern = indpattern;
 ...
 % In that cases, we want to keep our original estimation...which is
 % the case for SEM
 list_neighs = calculateNeighs(epattern,1);
 
 imagef = flip(images(icpattern).image,2);
 [npattern,p_k] = classifyPattern(list_neighs,imagef,codebookdir); % predict the letter name naively     
 j = 1;
...
points_list = fillPatternNames_SEM(all_cutpoints,epattern,icpattern,orientation);
